...
namespace TryCatchRemover
{
    public partial class FormTryCatchRemover : Form
    {
        ...
        private void button255TextImageLogo_Click(object sender, EventArgs e)
        {
            ...
            Bitmap d = new Bitmap(AI.Width, AI.Height);

            for (int i = 0; i < AI.Width; i++)
            {
                for (int x = 0; x < AI.Height; x++)
                {
                    Color oc = AI.GetPixel(i, x);
...
namespace ImageTextDeepLearning
{
    ...
    public class AllKeyboardOfWorld
    {
                ...
                if (Do && KeyboardAllImage.Count == 0)
                {
                    ...
                    for (int i = 0; i < KeyboardAllStrings.Count; i++)
                    {
                        ...
                        if (fonts.Count > 0)
                        {
                            ...
                            //draw string
                            e.DrawString(KeyboardAllStrings[i], new Font(fonts[0], 1F), Brushes.Black, new Rectangle(0, 0, Width, Height));
        ...
        public char[] AllSortedImageMatrix(char[] d)
        {
            ...
            ConvertAllTempageToMatrix(KeyboardAllImage);
            int count = 0;
            for (int i = 0; i < KeyboardAllConjunctionMatrix.Count; i++)
            {


                dd[count] = ImageTextDeepLearning.Colleralation.GetCorrelation(KeyboardAllConjunctionMatrix[i], Width);
                count++;
            }
