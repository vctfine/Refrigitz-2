...
namespace OpenCL
{
    ...
    public class CLArgument<T> where T : struct
    {
        ...
        {
            return CLArgument<T>.CreateArray(data);
        }

        public static implicit operator CLArgument<T>(T value)
        {
            return CLArgument<T>.CreateValue(value);
        }
        ...
        internal ComputeMemory GenerateComputeMemory(ComputeContext context)
        {
            ...
            if (CopyBack)
            {
                return new ComputeBuffer<T>(context, (ComputeMemoryFlags)memoryflags, Data.Length);
            }
            else
            {
                return new ComputeBuffer<T>(context, (ComputeMemoryFlags)memoryflags, Data);
            }
    ...
    public class CLArgumentInfo
    {
        ...
        {
            arg = System.Text.RegularExpressions.Regex.Replace(arg, @"\s+", " "); //  __global   int => __global int
    ...
    public class OpenCL
    {
        ...
        public void SetParameter(params object[] arguments)
        {
            for(int i=0;i<arguments.Length;i++)
            {
                ...
                MethodInfo CLvarconstruct = typeof(CLArgument<>).MakeGenericType(argumenttype).GetMethod(varname, System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
        ...
        /// </summary>
        public void Execute(int Worksize, params object[] parameter)
        {
            Execute(0,Worksize, -1, parameter);
        }

        /// <summary>
        /// Ignores previously set parameters set by OpenCL.SetArgument, is slower than Execute
        /// </summary>
        public void Execute(int Offset,int Worksize,int Localsize, params object[] parameter)
        {
