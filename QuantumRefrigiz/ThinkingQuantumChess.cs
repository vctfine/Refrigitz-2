...
namespace QuantumRefrigiz
{
    ...
    public class ThinkingQuantumChess//: IDisposable
    {
        ...
        //Constructor
        public ThinkingQuantumChess(int iInde, int KindO, int CurrentAStarGredy, bool MovementsAStarGreedyHeuristicTFou, bool IgnoreSelfObject, bool UsePenaltyRegardMechnisa, bool BestMovment, bool PredictHurist, bool OnlySel, bool AStarGreedyHuris, bool Arrangments, int i, int j)
        {
            object O = new object();
            lock (O)
            {
        ...
        public void Clone(ref ThinkingQuantumChess AA)
        {
            ...
            lock (O)
            {
                ...
                if (AA == null)
                {
                    AA = new ThinkingQuantumChess(iIndex, Kind, CurrentAStarGredyMax, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsChanged, Row, Column//, Kind
                        );
                }
                ...
                for (int ii = 0; ii < 8; ii++)
                {
                    for (int jj = 0; jj < 8; jj++)
                    {
                        ...
                        if (ab)
                        {
                            ...
                            //Create new ThinkingQuantumChess Rule Object.
...
#pragma warning restore CS0219 // The variable 'NIs' is assigned but its value is never used
        ...
        public double HeuristicDistribution(bool Before, int[,] Tab, int Order, Color a, int RowS, int ColS, int RowD, int ColD)
        {
            ...
            lock (O)
            {
                ...
                if (Order == 1)
                {
                    ...
                    {
                        int Cor = 0;
                        Task<int> H2 = Task.Factory.StartNew(() => Cor = QuantumRefrigiz.Colleralation.GetCorrelationScore(TableInitiation, CloneATable(Tab), 8, Order));
                        H2.Wait();
                        H2.Dispose();
                    ...
                    if (!ab)
                    {
                        ...
                        {
                            int Cor = 0;
                            Task<int> H3 = Task.Factory.StartNew(() => Cor = QuantumRefrigiz.Colleralation.GetCorrelationScore(TableInitiation, CloneATable(Tab), 8, Order));
                            H3.Wait();
                            H3.Dispose();
                ...
                else
                {
                    ...
                    {
                        int Cor = 0;
                        Task<int> H4 = Task.Factory.StartNew(() => Cor = QuantumRefrigiz.Colleralation.GetCorrelationScore(TableInitiation, CloneATable(Tab), 8, Order));
                        H4.Wait();
                        H4.Dispose();
                        if (Cor < Colleralation)
                        {
                    ...
                    if (!((Tab[3, 4] < ObjectBrown && Tab[4, 3] < ObjectBrown && Tab[3, 3] < ObjectBrown && Tab[4, 4] < ObjectBrown)) && (!IsNumberOfObjecttIsLessThanThreashold(CloneATable(Tab), 25)))
                    {
                        ...
                        {
                            int Cor = 0;
                            Task<int> H5 = Task.Factory.StartNew(() => Cor = QuantumRefrigiz.Colleralation.GetCorrelationScore(TableInitiation, CloneATable(Tab), 8, Order));
                            H5.Wait();
                            H5.Dispose();
        ...
        public double ReturnHeuristicCalculartorDeeperKing(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].KingOnTable == null || AStarGreedy[k].KingOnTable[m] == null || AStarGreedy[k].KingOnTable[m].KingThinkingQuantum == null || AStarGreedy[k].KingOnTable[m].KingThinkingQuantum[0] == null || AStarGreedy[k].KingOnTable[m].KingThinkingQuantum[0].TableListKing == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].KingOnTable[m].KingThinkingQuantum[0].TableListKing.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].KingOnTable[m].KingThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public double ReturnHeuristicCalculartorDeeperMinister(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].MinisterOnTable == null || AStarGreedy[k].MinisterOnTable[m] == null || AStarGreedy[k].MinisterOnTable[m].MinisterThinkingQuantum == null || AStarGreedy[k].MinisterOnTable[m].MinisterThinkingQuantum[0] == null || AStarGreedy[k].MinisterOnTable[m].MinisterThinkingQuantum[0].TableListMinister == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].MinisterOnTable[m].MinisterThinkingQuantum[0].TableListMinister.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].MinisterOnTable[m].MinisterThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public double ReturnHeuristicCalculartorDeeperCastle(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].CastlesOnTable == null || AStarGreedy[k].CastlesOnTable[m] == null || AStarGreedy[k].CastlesOnTable[m].CastleThinkingQuantum == null || AStarGreedy[k].CastlesOnTable[m].CastleThinkingQuantum[0] == null || AStarGreedy[k].CastlesOnTable[m].CastleThinkingQuantum[0].TableListCastle == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].CastlesOnTable[m].CastleThinkingQuantum[0].TableListCastle.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].CastlesOnTable[m].CastleThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public double ReturnHeuristicCalculartorDeeperHourse(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].HoursesOnTable == null || AStarGreedy[k].HoursesOnTable[m] == null || AStarGreedy[k].HoursesOnTable[m].HourseThinkingQuantum == null || AStarGreedy[k].HoursesOnTable[m].HourseThinkingQuantum[0] == null || AStarGreedy[k].HoursesOnTable[m].HourseThinkingQuantum[0].TableListHourse == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].HoursesOnTable[m].HourseThinkingQuantum[0].TableListHourse.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].HoursesOnTable[m].HourseThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public double ReturnHeuristicCalculartorDeeperElephant(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].ElephantOnTable == null || AStarGreedy[k].ElephantOnTable[m] == null || AStarGreedy[k].ElephantOnTable[m].ElefantThinkingQuantum == null || AStarGreedy[k].ElephantOnTable[m].ElefantThinkingQuantum[0] == null || AStarGreedy[k].ElephantOnTable[m].ElefantThinkingQuantum[0].TableListElefant == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].ElephantOnTable[m].ElefantThinkingQuantum[0].TableListElefant.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].ElephantOnTable[m].ElefantThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public double ReturnHeuristicCalculartorDeeperSolider(int k, int m, int ii, int j, int Order, ref int HaveKilled)
        {
            ...
            try
            {
                if (AStarGreedy[k].SolderesOnTable == null || AStarGreedy[k].SolderesOnTable[m] == null || AStarGreedy[k].SolderesOnTable[m].SoldierThinkingQuantum == null || AStarGreedy[k].SolderesOnTable[m].SoldierThinkingQuantum[0] == null || AStarGreedy[k].SolderesOnTable[m].SoldierThinkingQuantum[0].TableListSolder == null)
                {
                    return Heuristic;
                }

                for (int jj = 0; jj < AStarGreedy[k].SolderesOnTable[m].SoldierThinkingQuantum[0].TableListSolder.Count; jj++)
                {
                    int hav = HaveKilled;
                    Task<double> th = Task.Factory.StartNew(() => Heuristic += AStarGreedy[k].SolderesOnTable[m].SoldierThinkingQuantum[0].ReturnHeuristicCalculartor(0, ii, jj, Order * -1, ref hav));
                    th.Wait();
                    th.Dispose();
                    HaveKilled = hav;
                }
        ...
        public void KingThinkingQuantumChess(ref int[] LoseOcuuredatChiled, ref int WinOcuuredatChiled, int DummyOrder, int DummyCurrentOrder, int[,] TableS, int RowSource, int ColumnSource, bool DoEnemySelf, bool PenRegStrore, bool EnemyCheckMateActionsString, int RowDestination, int ColumnDestination, bool Castle)
        {
            ...
            lock (O)
            {
                ...
                bool ab = false;
                Task<bool> th = Task.Factory.StartNew(() => ab = ThinkingQuantumChessRuleThinking(CloneATable(TableS), RowSource, ColumnSource, RowDestination, ColumnDestination));
                th.Wait();
                th.Dispose();
                if (ab)
                {
...
#pragma warning restore CS0219 // The variable 'Added' is assigned but its value is never used
        ...
        private bool ThinkingQuantumChessRuleThinking(int[,] TableS, int RowSource, int ColumnSource, int RowDestination, int ColumnDestination)
        {
            object O = new object();
            lock (O)
            {
                        ...
                        if (AchmazPure.Count > 0)
                        {
                            for (int i = 0; i < AchmazPure[0].Count; i++)
                            {
                                for (int j = 0; j < AchmazPure[0][i].Count; j++)
                                {
                                    ...
                                    {
                                        ThinkingQuantumChess t = new ThinkingQuantumChess(0, Kind, CurrentAStarGredyMax, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsChanged, AchmazPure[0][i][j][0], AchmazPure[0][i][j][1], OrderColor(Order), CloneATable(Table), 0, Order, false, 0, 0, Kind);
